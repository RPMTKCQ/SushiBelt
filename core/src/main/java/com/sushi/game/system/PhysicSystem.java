...
public class PhysicSystem  extends IteratingSystem implements EntityListener {
    ...
    public void update(float deltaTime) {
        ...
        float beta = this.interval - this.accumulator;
        for (int i = 0; i < getEntities().size(); ++i) {
            this.interpolateEntity(getEntities().get(i), beta);
        }
    ...
    private void interpolateEntity(Entity entity, float beta) {
        Transform transform = Transform.MAPPER.get(entity);
        Physic physic = Physic.MAPPER.get(entity);

        transform.getPosition().set(
            MathUtils.lerp(transform.getPosition().x, physic.getPreviousPosition().x, beta),
            MathUtils.lerp(transform.getPosition().y, physic.getPreviousPosition().y, beta)
        );
    }
